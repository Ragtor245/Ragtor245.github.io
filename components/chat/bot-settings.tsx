...
export function BotSettings({ botName, botAvatar, onSave }: BotSettingsProps) {
  const [name, setName] = useState(botName)
  const [avatar, setAvatar] = useState(botAvatar || "")
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="icon" className="text-gray-400 hover:text-white hover:bg-gray-800">
          ...
          <span className="sr-only">机器人设置</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="cosmic-card border-gray-700">
        <DialogHeader>
          <DialogTitle className="text-white">机器人设置</DialogTitle>
          <DialogDescription className="text-gray-400">自定义您的AI助手的名称和头像。</DialogDescription>
        </DialogHeader>
